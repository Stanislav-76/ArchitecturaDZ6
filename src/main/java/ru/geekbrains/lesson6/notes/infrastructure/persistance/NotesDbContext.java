...
public class NotesDbContext extends DbContext implements NotesDatabaseContext {
    ...
    public void add(Note item) {
        ((NotesDatabase)database).getNotesTable().getRecords().add(new NotesRecord(item.getTitle(), item.getDetails()));

    }

    public void add(String title, String details) {
        ((NotesDatabase)database).getNotesTable().getRecords().add(new NotesRecord(title, details));

    }

    @Override
    public void remove(Note item) {
        ((NotesDatabase)database).getNotesTable().getRecords().removeIf(record -> record.getDetails() == item.getDetails());
    }
